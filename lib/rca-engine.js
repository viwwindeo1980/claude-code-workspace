...
// ─── Shared RCA prompt ────────────────────────────────────────────────────────
function buildPrompt(cluster, sampleDefects, adoContext, platformContext, zenDeskContext) {
...
Root cause must reference specific RCT components. Corrective actions must name the exact service/file/script to change (e.g. "Execute the VSDB migration script", "Update CustomOptions.bsd", "Re-deploy LetterService container", "Add missing ExtFld column").${zenDeskContext ? " Where ZenDesk ticket data is available, incorporate customer-reported context and resolution details into the corrective actions." : ""} Preventive actions must include platform-specific testing and monitoring steps.`;
}
  ...
  if (process.env.AZURE_OPENAI_ENDPOINT && process.env.AZURE_OPENAI_KEY) {
    try {
      ...
      const response = await client.chat.completions.create({
        ...
        messages: [{ role: "user", content: buildPrompt(cluster, sampleDefects, adoContext, platformContext, zenDeskContext) }],
      });
