...
import { buildPlatformContext } from "./rct-knowledge.js";
import { buildZenDeskContext } from "./zendesk.js";
import { buildReleaseNotesContext } from "./release-notes.js";
...
// ─── Shared RCA prompt ────────────────────────────────────────────────────────
function buildPrompt(cluster, sampleDefects, adoContext, platformContext, zenDeskContext, releaseNotesContext) {
  const adoSection = adoContext ? `
=== ADO RESOLUTION DATA (from closed defects in this cluster) ===
RCA Categories: ${adoContext.rcaCategories}
Fix Types: ${adoContext.fixTypes}
Issue Types: ${adoContext.issueTypes}
Resolution Comments:
${adoContext.toString().split("Resolution Comments:")[1] || ""}

CRITICAL INSTRUCTIONS:
- Ground corrective actions DIRECTLY in the Resolution Comments above.
- Cite specific scripts, queries, config changes, or deployment steps described in the resolution data.
- Use RCA Categories as the primary root cause classification.
- Use Fix Types to determine the type of corrective action required.
- Do NOT produce generic corrective actions when specific resolution data exists.
` : "";

  const zenSection = zenDeskContext ? `\n${zenDeskContext}\n` : "";
  const rnSection  = releaseNotesContext ? `\n${releaseNotesContext}\n` : "";

  const rnInstruction = releaseNotesContext
    ? " Where Release Notes data is available, treat the documented Root Cause and Resolution as official ground truth — reference the specific issue IDs and use the exact fix language when forming corrective actions."
    : "";

  return `You are a senior software quality engineer with deep expertise in the RCT (Risk Control Technology) P&C insurance Loss Control platform.

Your task is to analyze the defect cluster below and generate a precise, platform-specific Root Cause Analysis. Use the RCT architecture context provided to make recommendations specific to the actual components, services, and patterns of this product.

${platformContext || ""}
${rnSection}
=== DEFECT CLUSTER TO ANALYZE ===
...
${adoSection}${zenSection}
=== SAMPLE DEFECTS ===
${sampleDefects}

=== OUTPUT REQUIREMENTS ===
Generate a structured RCA using ONLY the RCT-specific terminology, service names (RCI, JAWS, RDI, SAWS, LetterService, ImagingService, VSDB, etc.), and fix patterns applicable to this platform. Do NOT use generic application names.

Root cause must reference specific RCT components. Corrective actions must name the exact service/file/script to change (e.g. "Execute the VSDB migration script", "Update CustomOptions.bsd", "Re-deploy LetterService container", "Add missing ExtFld column").${zenDeskContext ? " Where ZenDesk ticket data is available, incorporate customer-reported context and resolution details into the corrective actions." : ""}${rnInstruction} Preventive actions must include platform-specific testing and monitoring steps.`;
}
...
export async function generateRCA(cluster, defects) {
  // ── Build sample defect text including ADO resolution fields ─────────────
  const sampleDefects = defects.slice(0, 5).map(d => {
    let text = `[${d.key}] ${d.summary}\n${d.description.slice(0, 300)}`;
    if (d.resolution_comments) text += `\nResolution: ${d.resolution_comments.slice(0, 300)}`;
    if (d.fix_type) text += `\nFix Type: ${d.fix_type}`;
    if (d.rca_category) text += `\nRCA Category: ${d.rca_category}`;
    return text;
  }).join("\n\n---\n\n");

  // ── Aggregate ADO fields across all cluster defects ───────────────────────
  const resolvedDefects = defects.filter(d => d.resolution_comments || d.rca_category || d.fix_type);
  let adoContext = null;
  if (resolvedDefects.length > 0) {
    const rcaCategories = [...new Set(resolvedDefects.map(d => d.rca_category).filter(Boolean))].join(", ") || "N/A";
    const fixTypes = [...new Set(resolvedDefects.map(d => d.fix_type).filter(Boolean))].join(", ") || "N/A";
    const issueTypes = [...new Set(resolvedDefects.map(d => d.issue_type).filter(Boolean))].join(", ") || "N/A";
    const commentLines = resolvedDefects
      .filter(d => d.resolution_comments)
      .slice(0, 5)
      .map(d => `  [${d.key}] ${d.resolution_comments.slice(0, 400)}`)
      .join("\n");
    adoContext = {
      rcaCategories,
      fixTypes,
      issueTypes,
      toString: () =>
        `RCA Categories: ${rcaCategories}\nFix Types: ${fixTypes}\nIssue Types: ${issueTypes}\nResolution Comments:\n${commentLines || "  (none available)"}`,
    };
  }

  // ── Build platform-specific context from RCT knowledge base ──────────────
  const platformContext = buildPlatformContext(cluster, defects);

  // ── Fetch ZenDesk ticket context for any referenced tickets ──────────────
  const zenDeskContext = await buildZenDeskContext(defects);

  // ── Match release notes for related officially-fixed issues ──────────────
  const releaseNotesContext = buildReleaseNotesContext(cluster, defects);
    ...
    try {
      ...
      const response = await client.chat.completions.create({
        ...
        tools: [{
          ...
          function: {
            name:        "generate_rca",
            description: "Generate a structured Root Cause Analysis document for a defect cluster in the RCT Loss Control platform",
            parameters:  RCA_SCHEMA,
          },
        }],
        tool_choice: { type: "function", function: { name: "generate_rca" } },
        messages: [{ role: "user", content: buildPrompt(cluster, sampleDefects, adoContext, platformContext, zenDeskContext, releaseNotesContext) }],
      });
      ...
      if (toolCall?.function?.arguments) {
        ...
        return {
          ...result,
          confidence_score: adoContext ? 0.96 : 0.92,
          generated_by: `azure-foundry/${deployment}`,
          generated_at: new Date().toISOString(),
          ado_fields_used: adoContext ? {
            rca_categories: adoContext.rcaCategories,
            fix_types: adoContext.fixTypes,
            issue_types: adoContext.issueTypes,
          } : null,
          platform_knowledge: true,
          zendesk_tickets_used: zenDeskContext ? true : false,
          release_notes_used:   releaseNotesContext ? true : false,
        };
      }
    } catch (err) {
