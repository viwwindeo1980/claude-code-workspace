...
// ─── Shared RCA prompt ────────────────────────────────────────────────────────
function buildPrompt(cluster, sampleDefects, adoContext) {
  const adoSection = adoContext ? `
ADO Resolution Data (from closed/resolved defects in this cluster):
${adoContext}

IMPORTANT INSTRUCTIONS for corrective actions:
- Base corrective actions DIRECTLY on the Resolution Comments above — cite what was actually done to fix the issue.
- Use the Fix Type(s) (${adoContext.fixTypes}) to classify the type of fix required.
- Use the Bug Category/Issue Type(s) (${adoContext.issueTypes}) to frame the root cause context.
- Use the RCA Category values (${adoContext.rcaCategories}) as the primary root cause classification.
- If Resolution Comments describe a specific script, config change, code fix, or deployment step — include that as a concrete corrective action.
- Do NOT generate generic or hypothetical corrective actions when specific resolution data is available.
` : "";
  ...
  if (process.env.AZURE_OPENAI_ENDPOINT && process.env.AZURE_OPENAI_KEY) {
    try {
      ...
      const response = await client.chat.completions.create({
        ...
        messages: [{ role: "user", content: buildPrompt(cluster, sampleDefects, adoContext) }],
      });
